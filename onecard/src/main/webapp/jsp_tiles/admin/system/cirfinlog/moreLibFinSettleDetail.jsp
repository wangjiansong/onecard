...
function exportmoreLibFinSettleDetailExcel(){
	var form1 = document.getElementById("form1");
	form1.action = "<c:url value='/admin/sys/cirfinlog/exportmoreLibFinSettleDetailExcel' />";
	form1.submit();
...
function search(){
	var form1 = document.getElementById("form1");
	form1.action = "<c:url value='/admin/sys/cirfinlog/moreLibFinSettleDetail' />";
	form1.submit();
...
<fieldset>
	...
	<p>
		<form action="moreLibFinSettleDetail" id="form1" method="POST" class="form-inline">
...
<button class="btn btn-info" type="button" onclick="exportmoreLibFinSettleDetailExcel();">导出Excel</button>
</p>
<table class="table table-bordered table-hover table-condensed table-striped">
	...
	<tbody>
		<c:forEach items="${pageList }" var="lib" >
			<tr>
				<td><c:out value="${lib.SIMPLENAME }"/></td>
				<td><c:out value="${lib.FEETYPENAME }"/></td>
				<c:forEach items="${libs }" var="li" varStatus="status">
					<c:set var="n1" value="LIBNAME${status.index }" />
				 	<td>
				 	<c:if test="${lib[n1]>0 }">
				 	 收取
				 	  <fmt:formatNumber value="${lib[n1] }" pattern="0.00"/>
				 	</c:if>
				 	<c:if test="${lib[n1]<0 }">
				 	 退还
				 	  <fmt:formatNumber value="${-lib[n1] }" pattern="0.00"/>
			    ...
			    <td>
			    <fmt:formatNumber value="${lib.TOTAL_FEE }" pattern="0.00"/>
			    </td>
			</tr>
		</c:forEach>
	</tbody>
...
<%-- <c:set var="pager_base_url">${BASE_URL}/moreLibFinSettleDetail?</c:set> --%>
